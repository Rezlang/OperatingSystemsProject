...
class Process:
    ...
    def recalcTau(self, alpha):
        self.tau = math.ceil( alpha * self.CPUBursts[self.turnaroundIndex] + (1-alpha) * self.tau)
        return self.tau
...
class manager:
    def __init__(self, processes, numCPUProc):
        ...
        self.readyQueue: [Tuple[int, Process]] = []
    ...
    def calculateTurnarounds(self):
        ...
        return ((CPUBound + IOBound) / sum(p.numBursts for p in self.generatedProcesses),
                CPUBound / sum(p.numBursts if p.isCPUBound else 0 for p in self.generatedProcesses),
...
# TODO: First number in parenthesis is for CPU-bound processes, the second is for IO-bound processes
    ...
    print("-- average turnaround time: {:.3f} ms ({:.3f} ms/{:.3f} ms)".format(turnarounds[0],turnarounds[1],turnarounds[2]), file=fp)
    ...
    print("-- number of preemptions: {:d} ({}/{})".format(sum(p.numPreemptions for p in PM.generatedProcesses),
                                                          sum(p.numPreemptions if p.isCPUBound else 0 for p in PM.generatedProcesses),
        ...
        if PM.activeProcess.status == Status.RUNNING:
            if PM.activeProcess.burstTimes[0][0] == 0:
                ...
                if time < 10000000 and len(PM.activeProcess.burstTimes) != 1:
                    ...
                    print("time {}ms: Recalculating tau for process {}: old tau {}ms ==> new tau {}ms ".format(time, PM.activeProcess.id, oldTau, PM.activeProcess.tau) ,end="")
        ...
        if PM.activeProcess.status == Status.RUNNING:
            ...
            if PM.activeProcess.timeRunning > timeLimit:
                if len(PM.readyQueue) != 0:
                    #preempt
                    if time < 10000000:
                        print("time ", time,"ms: Time slice expired; preempting process ", PM.activeProcess.id, " with ", PM.activeProcess.burstTimes[0][0],"ms remaining ", sep="", end="")
                        PM.printQueue()
                    PM.activeProcess.switchingTimer = contextSwitchTime / 2 - 1
                    PM.activeProcess.status = Status.SWITCHING_OUT
                    ...
                    if time < 10000000:
                        print("time ", time, "ms: Time slice expired; no preemption because ready queue is empty [Q <empty>]", sep="")
                    PM.activeProcess.timeRunning = 1
                    PM.activeProcess.burstTimes[0][0] -= 1
