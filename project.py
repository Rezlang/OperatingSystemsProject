...
class Process:
    def __init__(self):
        self.id = ''
        self.arrivalTime = 0
        self.burstTimes: List[List[int, int]] = []
...
def FirstComeFirstServe(processes, contextSwitchTime):
    ...
    print("time " , time , "ms: Process " + PM.activeProcess.id + " arrived; added to ready queue [Q "+PM.activeProcess.id+"]", sep='')
    done = False
    while(not done):
        ...
        if PM.activeProcess.status == Status.SWITCHING_IN:
            if PM.activeProcess.switchingTimer == 0:
                ...
                print("time ", time, "ms: Process "+PM.activeProcess.id +" started using the CPU for ", PM.activeProcess.burstTimes[0][0] ,"ms burst", end="", sep='')

                if len(PM.readyQueue) != 0:
                    print("[Q", [" ".join(x.id) for x in PM.readyQueue] , "]")
                else:
                    print("[Q <empty>]")
            else:
        ...
        if PM.activeProcess.status == Status.RUNNING:
            if PM.activeProcess.burstTimes[0][0] == 0:
                ...
                print("time ", time, "ms: Process "+PM.activeProcess.id +" completed a CPU burst; ", len(PM.activeProcess.burstTimes)-1," bursts to go", end="", sep='')
                ...
                print("time ", time,"ms: Process "+ PM.activeProcess.id + " switching out of CPU; blocking on I/O until time ", (time + PM.activeProcess.burstTimes[0][1]),end="", sep='')
                if len(PM.readyQueue) != 0:
                    print("[Q "+ [" ".join(x.id) for x in PM.readyQueue] + "]")
                else:
                    print("[Q <empty>]")
        ...
        for p in PM.generatedProcesses:
            if p.status == Status.WAITING:
                if p.burstTimes[0][1] == 0:
                    ...
                    print("time ",time,"ms: Process "+ p.id + " completed I/O; added to ready queue ", end="", sep='')
                    if len(PM.readyQueue) != 0:
        ...
        for p in PM.generatedProcesses:
            if p.arrivalTime <= time and p.status == Status.UNARRIVED:
                PM.readyQueue.append(p)
                print("time " , time , "ms: Process " + p.id + " arrived; added to ready queue [Q " ,[" ".join(x.id) for x in PM.readyQueue] ,"]", sep='')
                p.status = Status.READY
        done = True
        for p in PM.generatedProcesses:
...
def partOneOutput(processes: List[Process], numCPUProc: int) -> int:
    print('<<< PROJECT PART I -- process set (n=' + str(len(processes)) + ') with ' + str(numCPUProc) + ' CPU-bound ' + ("process >>>" if numCPUProc == 1 else "processes >>>"))
    for i in range(len(processes)):
        print(('CPU-bound process ' if processes[i].isCPUBound else 'I/O-bound process ') + str(processes[i].id) + ': arrival time ' + str(processes[i].arrivalTime) + 'ms; ' + str(len(processes[i].burstTimes)) + (' CPU burst:' if len(processes[i].burstTimes) == 1 else ' CPU bursts:'))
        for j in range(len(processes[i].burstTimes)):
            if j != len(processes[i].burstTimes) - 1:
                print('--> CPU burst ' + str(processes[i].burstTimes[j][0]) + 'ms --> I/O burst ' + str(processes[i].burstTimes[j][1]) + 'ms')
            else:
                print('--> CPU burst ' + str(processes[i].burstTimes[j][0]) + 'ms')
    return 0
...
def main(argv):
    ...
    for i in range(numProc):
        ...
        if p.isCPUBound:
            ...
            p.burstTimes.append([math.ceil(next_exp(lambda_, upperBound) * 4), 0])
        else:
            for j in range(numCPUBursts - 1):
    ...
    FirstComeFirstServe(processes, contextSwitchTime)
    return 0
